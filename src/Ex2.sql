...
BEGIN
    ...
    END IF;
END
Это функция, которую я запихнула в комментарии потому что она не выгружается в файл иначе. Так что ее надо положить в функции и тогда через селект все отлично работает (могу скрины кинуть, если что). В скобках запроса к функции выставляется число n, до которого нам и нужны четные числа*/
